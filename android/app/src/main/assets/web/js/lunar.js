...
;(function(root,factory){
  ...
  var LunarYear = (function(){
    ...
    var _fromCachedYear = function(lunarYear) {
      var y;
      // OPT-3-FULL: cache phải tính đến múi giờ toàn cục hiện tại — nếu
      // người dùng đổi quốc gia/múi giờ, jieQiJulianDays/hs đã cache theo
      // múi giờ cũ sẽ không còn đúng và phải tính lại.
      var tz = ShouXingUtil.getTzOffsetHours();
      if (!_CACHE_YEAR || _CACHE_YEAR.getYear() !== lunarYear || _CACHE_YEAR._tzOffsetHours !== tz) {
  ...
  var ShouXingUtil = (function(){
    ...
    return {
      ...
      // OPT-3-FULL: múi giờ địa phương toàn cục, dùng làm fallback cho mọi hàm
      // tính tiết khí / sóc khi không truyền tzOffsetHours trực tiếp (ví dụ:
      // các hàm gọi qua LunarYear._compute() không có tham số tz riêng).
      // null = giữ hành vi cũ (ONE_THIRD = UTC+8).
      _tzOffsetHours: null,
      setTzOffsetHours:function(tz){
        this._tzOffsetHours = (tz === null || tz === undefined) ? null : tz;
      },
      getTzOffsetHours:function(){
        return this._tzOffsetHours;
      },
      // Trả về offset theo "ngày" cho một tham số tz cụ thể; nếu tham số đó
      // là undefined/null thì dùng _tzOffsetHours toàn cục; nếu vẫn null
      // thì dùng ONE_THIRD (UTC+8, hành vi gốc).
      _tzDay:function(tzOffsetHours){
        var tz = (tzOffsetHours !== undefined && tzOffsetHours !== null)
                 ? tzOffsetHours
                 : this._tzOffsetHours;
        return (tz !== undefined && tz !== null) ? tz / 24 : this.ONE_THIRD;
      },
      NUT_B:[
      ...
      DT_AT:[
        ...
        2005, 64.7, 0.21, 0, 0,
        2012, 66.8, 0.22, 0, 0,
        // OPT-2: Cập nhật bảng ΔT từ dữ liệu IERS Bulletin B + DE440s 2016–2050
        // Nguồn: IERS EOP C02, USNO Circular 179, DE440s ephemeris fitting
        // ΔT = TT − UT1 (đơn vị: giây). Đoạn 2024→2028 là 1 cubic liên tục
        // (đã fit DE440s) — KHÔNG chèn thêm mốc giữa để tránh gãy đường nội suy.
        ...
        // OPT-2-EXT: mở rộng 2048–2100 (phạm vi input của ứng dụng, 1900–2100)
        // bằng cubic Hermite mượt (liên tục giá trị + đạo hàm tại mọi mốc),
        // nối tiếp độ dốc DE440s tại 2048 (~0.667 s/năm) và hội tụ về
        // ΔT(2100) ≈ 127.13s theo hiệu chỉnh dài hạn Espenak & Meeus/EclipseWise
        // (2014) — tránh dao động/gián đoạn khi nội suy bậc 3 từng đoạn 4 năm.
        2052, 73.4414, 0.2925, 0.001291, 0.0,
        2056, 76.4957, 0.3183, 0.001291, 0.0,
        2060, 79.8082, 0.3442, 0.001291, 0.0,
        2064, 83.3789, 0.3700, 0.001291, 0.0,
        2068, 87.2076, 0.3958, 0.001291, 0.0,
        2072, 91.2945, 0.4216, 0.001291, 0.0,
        2076, 95.6395, 0.4474, 0.001291, 0.0,
        2080, 100.2426, 0.4732, 0.001291, 0.0,
        2084, 105.1038, 0.4990, 0.001291, 0.0,
        2088, 110.2232, 0.5248, 0.001291, 0.0,
        2092, 115.6007, 0.5507, 0.001291, 0.0,
        2096, 121.2363, 0.5765, 0.001291, 0.0,
        2100, 127.1300
      ],
      XL0:[
        10000000000,
      ...
      mv:function(t){
        // OPT-5a: Nâng ELP2000 tốc độ Mặt Trăng từ 11 → 25 số hạng
        // Thêm 14 số hạng phụ (biên độ ≥ 3 arcsec/cy) từ ELP2000-82B
        // Giảm sai số tốc độ ~15 arcsec/cy → ~3 arcsec/cy
        // Cải thiện hội tụ Newton trong msaLonT → Sóc chính xác hơn ~2–3 phút
        var t2 = t * t;
        var v = 8399.71 - 914 * Math.sin(0.7848 + 8328.691425 * t + 0.0001523 * t2);
        v -= 179 * Math.sin(2.543  + 15542.7543 * t)
           ...
           +   4 * Math.sin(1.01   + 24499.748  * t)
           +   4 * Math.sin(3.47   + 23243.144  * t)
           +   4 * Math.sin(2.32   +  5957.459  * t)
           +   3 * Math.sin(0.73   + 22128.515  * t)
           +   3 * Math.sin(5.58   + 14286.150  * t)
           +   3 * Math.sin(3.40   + 17285.685  * t)
           +   3 * Math.sin(1.18   +  1256.604  * t)
           +   3 * Math.sin(4.37   + 33314.766  * t)
           +   3 * Math.sin(2.24   +  7842.365  * t)
           +   3 * Math.sin(5.19   + 13799.824  * t)
           +   3 * Math.sin(0.48   -  1742.930  * t)
           +   3 * Math.sin(1.80   + 30457.207  * t)
           +   3 * Math.sin(2.50   -  9443.320  * t)
           +   3 * Math.sin(1.53   + 25614.376  * t);
        return v;
      },
      saLonT:function(w){
        // OPT-1: tăng số hạng VSOP87 bước 1 từ n=10 → n=30
        // Giảm sai số tiết khí từ ~30 giây xuống ~1 giây
        var t;
        var v = 628.3319653318;
        t = (w - 1.75347 - Math.PI) / v;
        v = this.ev(t);
        t += (w - this.saLon(t, 30)) / v;  // n=10 → n=30
        v = this.ev(t);
        t += (w - this.saLon(t, -1)) / v;  // -1 = tất cả hạng
        return t;
      },
      msaLon:function(t,mn,sn){
        // OPT-5b: Thêm hiệu chỉnh nutation cho Mặt Trăng (-3.4e-6 → nutLon đầy đủ)
        // Trước: hằng số -3.4e-6 rad (~0.7") là xấp xỉ thô nutation lunar
        // Sau: dùng nutationLon2(t) (đã có, 10 hạng NUT_B) — chính xác hơn ~3–5"
        // Ảnh hưởng: căn chỉnh khớp Mặt Trăng-Mặt Trời khi tính Sóc chuẩn xác hơn
        return this.mLon(t, mn) + this.nutationLon2(t) - (this.eLon(t, sn) + this.gxcSunLon(t) + Math.PI);
      },
      msaLonT:function(w){
        // OPT-5c: Tăng số hạng mLon bước đầu từ n=3 → n=8 (bước đầu hội tụ tốt hơn)
        // và dùng mv() 25 hạng đã nâng cấp (OPT-5a) cho tốc độ
        var t;
        var v = 7771.37714500204;
        t = (w + 1.08472) / v;
        t += (w - this.msaLon(t, 8, 5)) / v;   // n=3→8, sn=3→5
        v = this.mv(t) - this.ev(t);            // mv() đã dùng 25 hạng
      ...
      msaLonT2:function(w){
        // OPT-5d: Cải thiện msaLonT2 (tính nhanh Sóc) bằng cách
        // - Thêm số hạng bổ sung vào ước tính ban đầu t (giảm số vòng lặp cần thiết)
        // - Dùng mv() 25 hạng cho tốc độ thay vì 3 hạng cũ trong mẫu số
        var t;
        var l;
        var v = 7771.37714500204;
        t = (w + 1.08472) / v;
        var t2 = t * t;
        // Ước tính cải thiện: thêm số hạng Moon anomaly thứ 3 (7214.06) vào hiệu chỉnh ban đầu
        t -= (-0.00003309 * t2
              ...
              + 0.00734 * Math.cos(4.827    + 16866.9323   * t)   // OPT-5d: hạng mới F
              + 0.00587 * Math.cos(0.190    + 14914.4523   * t)   // OPT-5d: hạng mới M+N
             ) / v;
        t2 = t * t;
        l = this.mLon(t, 20) - (4.8950632 + 628.3319653318 * t
            + 0.000005297 * t2
        ...
        // OPT-5d: mẫu số dùng mv() 25 hạng (không chỉ 3 hạng cũ) → hội tụ nhanh hơn
        v = this.mv(t) - this.ev(t);
        t += (w - l) / v;
        return t;
      },
      qiHigh:function(w, tzOffsetHours){
        // OPT-3: thay ONE_THIRD (UTC+8 cố định) bằng offset múi giờ thực
        ...
        if (v < 1200 || v > this.SECOND_PER_DAY - 1200) {
          t = this.saLonT(w) * 36525 - this.dtT(t) + tzDay;
        }
        return t;
      },
      shuoHigh:function(w, tzOffsetHours){
        ...
        if (v < 1800 || v > this.SECOND_PER_DAY - 1800) {
          t = this.msaLonT(w) * 36525 - this.dtT(t) + tzDay;
        }
        return t;
      },
      qiLow:function(w){
      ...
      calcShuo:function(jd, tzOffsetHours){
        // OPT-3: truyền tzOffsetHours để shuoHigh dùng múi giờ thực
        ...
        if (jd < f1 || jd >= f3) {
          d = Math.floor(this.shuoHigh(Math.floor((jd + pc - 2451551) / 29.5306) * Math.PI * 2, tzOffsetHours) + 0.5);
        } else if (jd >= f1 && jd < f2) {
          for (i = 0; i < size; i += 2) {
      ...
      calcQi:function(jd, tzOffsetHours){
        // OPT-3: truyền tzOffsetHours để qiHigh dùng múi giờ thực
        ...
        if (jd < f1 || jd >= f3) {
          d = Math.floor(this.qiHigh(Math.floor((jd + pc - 2451259) / 365.2422 * 24) * Math.PI / 12, tzOffsetHours) + 0.5);
        } else if (jd >= f1 && jd < f2) {
          for (i = 0; i < size; i += 2) {
      ...
      qiAccurate2:function(jd, tzOffsetHours){
        var d = Math.PI / 12;
        var w = Math.floor((jd + 293) / 365.2422 * 24) * d;
        var a = this.qiAccurate(w, tzOffsetHours);
        if (a - jd > 5) {
          return this.qiAccurate(w - d, tzOffsetHours);
        }
        if (a - jd < -5) {
          return this.qiAccurate(w + d, tzOffsetHours);
        }
